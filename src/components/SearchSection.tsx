    ...
    <section className="sticky backdrop-blur-lg top-20 z-40 shadow-md bg-white dark:bg-transparent m-3 p-3 shadow-md border border-gray-700 rounded-2xl w-[90vw]">
      <div className="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0 md:space-x-4">
        ...
        <select
          ...
          className="p-3 border border-gray-700 dark:border-gray-600 rounded-xl dark:bg-transparent bg-gray-100 dark:text-white text-black w-full md:w-1/4"
        >
          {specializations.map((spec) => (
            <option key={spec} value={spec} className="bg-gray-700 dark:bg-gray-800 text-white">
              {spec}
            </option>
          ))}
        </select>
